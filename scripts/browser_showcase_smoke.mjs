...
if (!siteArg || !outputArg || !endpoint)
  throw new Error("usage: node browser_showcase_smoke.mjs SITE NEW_OUTPUT CDP_URL");
  ...
  try {
    ...
    if (fault === "abi" && name === "/pkg/manifest.json") {
      ...
      const changed = Buffer.concat([
        await readFile(`${site}/pkg/FrankenTerm.js`),
        incompatibleApi,
      ]);
...
const records = (start = 0) =>
  observations.console.slice(start).flatMap((text) => {
    try {
...
try {
  ...
  await send(
    ...
    { behavior: "allow", downloadPath: `${output}/downloads`, eventsEnabled: true },
    null,
  );
  ...
  await waitFor(
    async () => records().some((row) => row.event === "init") || Boolean(await evaluate(errorText)),
    "real startup",
    120000,
  );
  ...
  await waitFor(
    () => evaluate('getComputedStyle(document.getElementById("loading-overlay")).opacity === "0"'),
  ...
  await writeFile(
    `${output}/canvas.png`,
    Buffer.from(observations.display.canvasPng.split(",")[1], "base64"),
    { flag: "wx" },
  );
  ...
  await waitFor(
    () => records().some((row) => row.event === "resize" && row.changed),
    "resize admission",
  );
  await waitFor(() => {
    const rows = records();
    ...
    return (
      resize >= 0 &&
      rows.slice(resize + 1).some((row) => row.event === "frame" && row.frame_idx > frameIdx)
    );
  ...
  assert.ok(
    tail.some(
      (row) =>
        row.event === "input" &&
        row.data?.kind === "paste" &&
        row.data.text === "retained 你好 👩‍💻 {tail}",
    ),
    "exact paste survives quit",
  );
  ...
  await waitFor(
    () => observations.downloads.some((event) => event.params.state === "completed"),
    "keyboard download",
  );
  ...
  assert.deepEqual(
    burstTail.map((row) => row.data.text),
    Array.from({ length: 4095 }, (_, i) => `burst-${i} 你好 👩‍💻`),
    "every accepted paste survives in FIFO order",
  );
  const burstAdmission = records(burstStart).filter((row) => row.event === "input_admission");
  assert.equal(burstAdmission.length, 4098);
  assert.deepEqual(
    burstAdmission.map((row) => row.sequence),
    Array.from({ length: 4098 }, (_, i) => burstAdmission[0].sequence + i),
  );
  assert.ok(
    burstAdmission
      .slice(0, 4096)
      .every(
        (row) =>
          ...
          row.normalized.length === 1 &&
          row.normalized[0].outcome === "queued",
      ),
  );
  assert.deepEqual(
    burstAdmission.slice(4096).map((row) => row.outcome),
    ["rejected", "rejected"],
  );
  ...
  assert.ok(
    burstAdmission
      .slice(0, 4097)
      .every((row) => row.vt_chunks_discarded === 1 && row.ime_records_discarded === 0),
    "local VT copies drain on each call",
  );
  assert.equal(
    records(burstStart).find((row) => row.event === "step" && !row.running).events_processed,
    1,
    "only q is processed; paste tail is recovered",
  );
  ...
  assert.deepEqual(
    partialTail.slice(0, -1).map((row) => row.data.text),
    Array.from({ length: 4094 }, (_, i) => `partial-${i}`),
  );
  assert.deepEqual(partialTail.at(-1).data, { kind: "ime", phase: "start", text: "" });
  const partial = records(partialStart).find(
    (row) => row.event === "input_admission" && row.kind === "composition",
  );
  assert.equal(partial.outcome, "partial");
  assert.deepEqual(partial.normalized, [
    { index: 0, outcome: "queued" },
    { index: 1, outcome: "rejected" },
  ]);
  ...
  assert.deepEqual(
    byteAdmission.map((row) => row.outcome),
  ...
  assert.ok(
    byteAdmission
      ...
      .every(
        (row) =>
          row.reason === "text_too_large" &&
          row.normalized.length === 0 &&
          row.vt_chunks_discarded === 0,
      ),
  );
  ...
  assert.deepEqual(
    retryTail.map((row) => row.data.text),
    ["retry-tail 👩‍💻"],
  );
  const retryAdmission = records(retryStart).filter((row) => row.event === "input_admission");
  assert.equal(retryAdmission.length, 8);
  assert.ok(
    retryAdmission.every(
      (row) => row.outcome === "accepted" && row.normalized[0].outcome === "queued",
    ),
  );
  const retrySteps = records(retryStart).filter((row) => row.event === "step");
  assert.deepEqual(
    retrySteps.map((row) => row.events_processed),
    [2, 2, 3],
  );
  assert.deepEqual(
    retrySteps.map((row) => row.rendered),
    [true, true, false],
  );
  const retryRecords = records(retryStart);
  for (const step of retrySteps.filter((row) => row.rendered)) {
    const start = retryRecords.findIndex(
      (row) => row.event === "step" && row.frame_idx === step.frame_idx,
    );
    const remaining = retryRecords.slice(start + 1);
    const frame = remaining.findIndex(
      (row) => row.event === "frame" && row.frame_idx === step.frame_idx,
    );
    const next = remaining.findIndex(
      (row) => row.event === "step" || row.event === "input_admission",
    );
    assert.ok(
      frame >= 0 && (next < 0 || frame < next),
      "each intermediate frame is presented before the retry completes",
    );
  }
  ...
  for (const phase of ["update", "end"]) {
    ...
    if (phase === "update")
      assert.ok(
        imeAdmission.some(
          (row) => row.reason === "composition_cancelled" && row.normalized.length === 0,
        ),
        "failed session's trailing end is rejected",
      );
    assert.ok(
      imeAdmission.some((row) => row.kind === "key" && row.normalized[0]?.outcome === "queued"),
      "q is not suppressed by stale composition state",
    );
    assert.equal(
      records(imeStart).find((row) => row.event === "step" && !row.running).events_processed,
      4,
      "start/update/cancel/q only; stale preedit is never committed",
    );
  ...
  assert.deepEqual(
    ...
    [],
    "all positive admission journeys have no uncaught errors",
  );
  ...
  const screenLogs = (start) =>
    records(start).filter((row) => row.event === "log" && row.text.startsWith("[determinism]"));
  const checksumPattern =
    /^\[determinism\] checksum \((Full|DirtyRows|FullRedraw)\) live = 0x[0-9a-f]{16}$/;
  await waitFor(
    () =>
      records(screenLogStart).some((row) => row.event === "frame" && row.events_processed === 14),
    "screen navigation and log input processed",
  );
  ...
  await waitFor(
    () => records(quitLogStart).some((row) => row.event === "quit"),
    "screen logs followed by quit",
  );
  ...
  assert.ok(
    !quitRows.slice(stoppedIndex).some((row) => row.event === "frame"),
    "quit logs must not require a frame",
  );
  const finalLogs = screenLogs(quitLogStart);
  assert.deepEqual(
    finalLogs.map((row) => row.text.match(checksumPattern)?.[1]),
    ["Full", "DirtyRows", "FullRedraw"],
  );
  ...
  await send("Page.navigate", {
    url: `${url.replace("jsonl=1", "jsonl=0")}&scenario=stopped-notice`,
  });
  await waitFor(
    () =>
      evaluate(
        'location.search.includes("jsonl=0") && document.getElementById("loading-overlay") && document.getElementById("status") && getComputedStyle(document.getElementById("loading-overlay")).opacity === "0" && document.getElementById("status").textContent.includes("panes")',
      ),
    "quiet showcase startup",
  );
  ...
  assert.deepEqual(
    quietTail.map((row) => row.data.text),
    ["quiet accepted tail"],
  );
  await evaluate(`(() => {
  ...
  assert.equal(
    await evaluate('document.getElementById("input-rejection-count")?.textContent.trim() || ""'),
    "— 1 input rejected",
  );
  ...
  for (const name of ["missing", "corrupt", "revision", "abi"]) {
    ...
    if (name === "abi")
      assert.match(await evaluate(errorText), /Unsupported first-party renderer API contract/);
    ...
    if (name === "revision")
      assert.match(await evaluate(errorText), /Unsupported renderer source revision/);
    if (name === "corrupt")
      assert.ok(
        observations.browserLog.slice(logStart).some((row) => /integrity|digest/i.test(row.text)),
        "valid modified JS must fail byte integrity",
      );
    if (name === "missing")
      assert.ok(
        observations.browserLog.slice(logStart).some((row) => /404/.test(row.text)),
        "missing package must return 404",
      );
