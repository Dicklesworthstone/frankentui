...
/// | `enable_bocpd` | `false` | the rate heuristic is the detector |
...
pub struct CoalescerConfig {
    ...
    /// Enable BOCPD (Bayesian Online Change-Point Detection) for regime
    /// detection. **Off by default**: the rate threshold heuristic
    /// (`burst_enter_rate` / `burst_exit_rate`) is the detector unless
    /// [`Self::with_bocpd`] turns the posterior on.
    ///
    /// When enabled, the coalescer uses a Bayesian posterior over run-lengths
    /// to detect regime changes instead of the thresholds, which buys
    /// uncertainty quantification via P(burst), adaptation without hand-tuned
    /// rates, and per-decision evidence rows.
    ///
    /// # Why it is not the default
    ///
    /// It was, from 2026-09-02. The differential replay that was supposed to
    /// justify that flip landed two and a half weeks later and said the flip
    /// was not justified: driven over identical schedules on a virtual clock,
    /// the posterior drew **more** frames during a drag on 6 of 6 recorded
    /// storm patterns, and was no better on settling latency (each detector
    /// over the 40 ms budget on 3 of 6). The plan's own rule for that outcome
    /// was to restore the default rather than lower the bar.
    ///
    /// The likely cause is that `recommended_delay` interpolates between the
    /// steady and burst holds while `p_burst` sits in the transitional band,
    /// so on storms that straddle `mu_burst` it holds for an intermediate
    /// delay instead of committing — less holding, more repaints. Retuning
    /// that is open as a separate question, not a reason to default it on.
    ///
    /// Evidence: `docs/perf/resize_differential_2026-09.md`, regenerated by
    /// `scripts/resize_storm_differential_e2e.sh`. bd-h8l3d.
...
impl Default for CoalescerConfig {
    fn default() -> Self {
        Self {
            ...
            // The differential replay that was meant to justify making the
            // posterior the default said it loses; see the field's doc.
            enable_bocpd: false,
...
mod tests {
    ...
    /// The hard deadline holds with the posterior on.
    ///
    /// `hard_deadline_forces_apply` above proves it for the heuristic, which
    /// is what ships. This one covers the opt-in path: a 100 ms SLA that
    /// quietly stopped applying under `with_bocpd()` would be a regression
    /// nothing else here would catch.
    #[test]
    fn hard_deadline_still_forces_apply_with_bocpd_on() {
        let config = CoalescerConfig::default().with_bocpd().with_logging(true);
        assert!(config.enable_bocpd, "the detector under test is on");
        let mut c = ResizeCoalescer::new(config, (80, 24));
    ...
    fn steady_regime_delay_is_16ms_and_burst_is_40ms() {
        // The thresholds this documents are the posterior's, so opt in.
    ...
    fn decision_evidence_json_has_detector_and_p_burst() {
        // `p_burst` only exists when the posterior is running.
        let config = CoalescerConfig::default().with_bocpd().with_logging(true);
        let mut c = ResizeCoalescer::new(config, (80, 24));
    ...
    mod property {
        ...
        proptest! {
            ...
            fn proptest_transitions_alternate_and_tags_match_path(
                ...
                // Two of the three claims are about BOCPD-tagged rows, so the
                // posterior has to be running for this to check anything.
                let mut c = ResizeCoalescer::new(
    ...
    /// The posterior ships off. It was the default from 2026-09-02 until the
    /// differential replay it was waiting on measured it drawing more frames
    /// during a drag on 6 of 6 storm patterns, and no better on settling
    /// latency — see `enable_bocpd`'s doc and bd-h8l3d. Turning it back on
    /// should change this assertion and the evidence together.
    #[test]
    fn config_default_leaves_bocpd_off_with_heuristic_fallback() {
        let config = CoalescerConfig::default();
        ...
        assert!(c.bocpd().is_none(), "no posterior is built when it is off");
        ...
        // Opting in still works and is what the evidence was gathered under.
        let on = ResizeCoalescer::new(CoalescerConfig::default().with_bocpd(), (80, 24));
        assert!(on.bocpd_enabled());
        assert!(on.bocpd().is_some());
        assert!(on.bocpd_p_burst().is_some());
    }
    ...
    fn bocpd_mode_exits_burst_on_idle_tick() {
        let mut c = ResizeCoalescer::new(CoalescerConfig::default().with_bocpd(), (80, 24));
        let base = Instant::now();
        let mut now = base;
        for i in 0..40u16 {
