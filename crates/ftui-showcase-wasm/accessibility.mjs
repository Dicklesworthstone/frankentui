...
class TerminalContentReview {
  constructor(bridge, terminal, keyboardProxy) {
    ...
    let id;
    do { id = `ftui-content-review-${++showcaseReviewSequence}`; }
    ...
    this.controls.style.cssText = 'position:fixed;top:0.5rem;right:0.5rem;z-index:20;font:14px system-ui;';
    ...
    this.hint.textContent = 'Press Alt+Shift+R to read or copy terminal content. In content review, Escape returns to the terminal and Tab moves between browser controls.';
    ...
    this.panel.style.cssText = 'position:fixed;inset:12vh 2vw auto;max-height:80vh;overflow:auto;z-index:1000;padding:1rem;border:2px solid currentColor;background:Canvas;color:CanvasText;font:16px system-ui;box-sizing:border-box;';
    const heading = this.document.createElement('h2');
    heading.textContent = 'Terminal content review';
    const help = this.document.createElement('p');
    help.id = `${id}-help`;
    help.textContent = 'This read-only snapshot stays fixed while you read. Select and copy normally. Refresh to read newer content. Escape or Return to terminal closes this panel; Tab can leave it.';
    ...
    this.text.style.cssText = 'box-sizing:border-box;width:100%;height:45vh;font:16px monospace;background:Canvas;color:CanvasText;';
    ...
    this.inputEvents = ['keydown', 'keyup', 'beforeinput', 'input', 'paste', 'copy', 'cut',
    ...
    const describedBy = (terminal.getAttribute('aria-describedby') || '').split(/\s+/).filter(Boolean);
    terminal.setAttribute('aria-describedby', [...describedBy, this.hint.id].join(' '));
  }

  owns(target) {
    return target instanceof this.window.Node
      && (this.controls.contains(target) || this.panel.contains(target));
  }

  isTerminal(target) {
    return target === this.terminal || (this.keyboardProxy !== null && target === this.keyboardProxy);
  }
  ...
  routeInput(event) {
    ...
    const shortcut = event.type === 'keydown' && !event.isComposing
  ...
  updateStatus() {
    const status = this.latest?.text === this.text.value
      ? 'Snapshot is up to date.' : 'New terminal content is available. Refresh when ready.';
  ...
  close(restoreFocus) {
    ...
    this.returnTarget = null;
    if (restoreFocus && ownedFocus && !this.document.hidden && target?.isConnected
        && !target.disabled) target.focus({ preventScroll: true });
  }
  ...
  clear() {
    ...
    this.text.value = '';
    this.status.textContent = '';
    if (openerFocused && !this.document.hidden && this.terminal.isConnected
        && !this.terminal.disabled) this.terminal.focus({ preventScroll: true });
    this.openButton.disabled = true;
  }
  ...
  dispose() {
    ...
    const ids = (this.terminal.getAttribute('aria-describedby') || '').split(/\s+/)
...
export class ShowcaseAccessibilityBridge {
  constructor(root, { terminal = null, keyboardProxy = null } = {}) {
    ...
    if (terminal?.ownerDocument === this.document && terminal.isConnected
        && typeof terminal.focus === 'function' && !root.contains(terminal)) {
  ...
  flush() {
    this.timer = null;
    if (this.disposed || !this.enabled || this.document.hidden || this.review?.opened
...
export function attachShowcaseAccessibility() {
  ...
  return root ? new ShowcaseAccessibilityBridge(root, {
