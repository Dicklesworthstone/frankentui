...
export function browserCases(api) {
  ...
  const speech = (text, urgency = "polite", node_id = "1") => ({
    ...
    urgency,
    node_id,
    reason: "FocusedStateChanged",
  });
  const update = (frame, overrides = {}) =>
    JSON.stringify({
      schema_version: 1,
      enabled: true,
      frame_id: String(frame),
      focus_id: "1",
      lines: ["textInput: Email. focused"],
      omitted_nodes: 0,
      announcements: [],
      dropped_count: 0,
      ...overrides,
  ...
  return (async () => {
    ...
    await run(
      "text is literal, Unicode is preserved, and DOM focus never moves",
      async (bridge, root) => {
        const control = document.getElementById("focus-target");
        control.focus();
        const text = '<img src=x onerror="window.injected=true"> é界🦀';
        check(
          bridge.update(update(0, { lines: [text], announcements: [speech(text)] })),
          "update refused",
        );
        await tick();
    ...
    await run(
      "equal text from different nodes and later transitions is not deduplicated",
      async (bridge) => {
        bridge.update(
          update(0, {
            announcements: [speech("selected", "polite", "1"), speech("selected", "polite", "2")],
          }),
        );
        await tick();
    ...
    await run(
      "malformed and oversized packets leave DOM and pending speech unchanged",
      async (bridge, root) => {
        bridge.update(update(0));
        const html = root.innerHTML;
        for (const payload of [
          "{broken",
          update(1, { schema_version: 2 }),
          update(1, { frame_id: 1 }),
          update(1, { lines: Array(129).fill("x") }),
          update(1, { lines: ["x".repeat(241)] }),
          ...
          update(1, { dropped_count: -1 }),
          update(1, { focus_id: 9007199254740992 }),
          " ".repeat(250001),
        ])
    ...
    await run(
      "all pending and live history remain bounded, preserving urgent speech first",
      async (bridge) => {
    ...
    await run(
      "hidden tabs clear queued speech and never replay background updates",
      async (bridge) => {
        ...
        bridge.update(
          update(1, { lines: ["background mirror"], announcements: [speech("background")] }),
        );
        await tick();
        check(
          bridge.pending.length === 0 && bridge.polite.children.length === 0,
          "hidden speech queued",
        );
    ...
    await run(
      "disable clears content, rejects stale disable, and permits a fresh enabled frame",
      async (bridge, root) => {
        ...
        check(
          !bridge.update(update(2, { announcements: [speech("stale private")] })),
          "disabled frame replayed",
        );
        check(
          bridge.update(update(3, { announcements: [speech("fresh")] })),
          "fresh enable refused",
        );
        await tick();
    ...
    await run(
      "packaged runner enables before init and publishes the initial frame once",
      async (_, root) => {
        ...
        try {
          ...
          await tick();
          check(
            root.querySelector('[role="log"][aria-live="polite"]').textContent === "initial focus",
            "no first-frame DOM speech",
          );
        } finally {
          runner.free();
        }
    ...
    await run(
      "step-before-init preserves first speech across the immediate quiet render",
      async (_, root) => {
    ...
    await run(
      "explicit runner configuration chooses manual delivery without double speech",
      async (_, root) => {
        const runner = new (api.withShowcaseAccessibility(RunnerContract))();
        try {
    ...
    await run(
      "runner destroy and free cancel DOM tasks and retain underlying cleanup",
      async (_, root) => {
        const Runner = api.withShowcaseAccessibility(RunnerContract);
    ...
    check(
      api.attachShowcaseAccessibility() === null,
      "missing proxy should not create a body node",
    );
    return results;
  })();
}
...
export async function browserReviewCases(api) {
  ...
  await run(
    "review is discoverable outside the clipped proxy and waits for a snapshot",
    async ({ bridge, review, root, canvas }) => {
      check(review !== null, "terminal review not attached");
      check(
        !root.contains(review.controls) && !root.contains(review.panel),
        "controls inherit proxy clipping",
      );
      ...
      check(
        review.openButton.getAttribute("aria-keyshortcuts") === "Alt+Shift+R",
        "shortcut missing",
      );
      check(
        review.openButton.getAttribute("aria-controls") === review.panel.id,
        "broken controls relation",
      );
      check(
        canvas.getAttribute("aria-describedby").includes(review.hint.id),
        "terminal exit hint missing",
      );
      check(
        canvas.getAttribute("aria-describedby").includes("existing-help"),
        "host description replaced",
      );
  ...
  await run(
    "review opens a literal bounded snapshot and announces its native readonly semantics",
    async ({ bridge, review, canvas, root }) => {
      ...
      check(
        review.text.value === `${literal}\n7 additional items omitted.`,
        "snapshot changed or truncation hidden",
      );
      check(review.openButton.getAttribute("aria-expanded") === "true", "open state not exposed");
      check(
        !review.panel.hasAttribute("aria-modal"),
        "review incorrectly traps the page as a modal",
      );
  ...
  await run(
    "review freezes selection and content until an explicit refresh",
    async ({ bridge, review, canvas }) => {
      ...
      check(
        review.status.textContent.includes("New terminal content"),
        "new content not discoverable",
      );
      review.refreshButton.click();
      check(review.text.value === "replacement snapshot", "refresh not applied");
      check(
        review.text.selectionStart === 2 && review.text.selectionEnd === 6,
        "refresh discarded selection",
      );
      ...
      check(
        review.text.selectionStart === 2 && review.text.selectionEnd === 6,
        "unchanged refresh reset selection",
      );
    },
  );

  await run(
    "review suppresses concurrent speech without replaying it on return",
    async ({ bridge, review, canvas }) => {
  ...
  await run(
    "review isolates input from host handlers without canceling native defaults",
    async ({ bridge, review, canvas, outside }) => {
      bridge.update(packet(0));
      canvas.focus();
      const seen = [];
  ...
  await run(
    "Escape returns focus and consumes its release on the terminal",
    async ({ bridge, review, canvas }) => {
      bridge.update(packet(0));
      canvas.focus();
      review.open();
      ...
      try {
        ...
        check(
          review.panel.hidden && document.activeElement === canvas,
          "Escape did not return to canvas",
        );
  ...
  await run(
    "review respects IME, repeat, unrelated input and mobile focus ownership",
    async ({ bridge, review, keyboardProxy, outside, canvas }) => {
      bridge.update(packet(0));
      const chord = (extra) =>
        new KeyboardEvent("keydown", {
          key: "R",
      ...
      check(
        review.opened && document.activeElement === review.text,
        "mobile input cannot enter review",
      );
      review.close(true);
  ...
  await run(
    "disable erases review copies and reenable starts with a fresh snapshot",
    async ({ bridge, review, canvas }) => {
      ...
      check(
        review.panel.hidden && review.text.value === "" && review.latest === null,
        "disable retained private snapshot",
      );
      check(
        review.openButton.disabled && document.activeElement === canvas,
        "disable left focus hidden",
      );
  ...
  await run(
    "replacing a runner removes old review controls, listeners and descriptions",
    async ({ bridge: old, review, canvas, root }) => {
      old.update(packet(0));
      canvas.focus();
      review.open();
      ...
      canvas.setAttribute(
        "aria-describedby",
        `${canvas.getAttribute("aria-describedby")} concurrent-help`,
      );
      const fresh = api.attachShowcaseAccessibility();
      try {
        ...
        check(
          !canvas.getAttribute("aria-describedby").split(/\s+/).includes(oldHint),
          "old hint survived",
        );
        check(
          canvas.getAttribute("aria-describedby").includes("concurrent-help"),
          "host hint lost",
        );
        old.dispose();
      ...
      check(
        canvas.getAttribute("aria-describedby") === "existing-help concurrent-help",
        "description cleanup changed host tokens",
      );
    },
  );

  await run(
    "disposal returns owned focus without stealing unrelated focus and cancels all UI",
    async ({ bridge, review, canvas, outside }) => {
      bridge.update(packet(0));
      canvas.focus();
      review.open();
  ...
  await run(
    "returning directly to canvas closes review without replay or hidden focus",
    async ({ bridge, review, canvas }) => {
      bridge.update(packet(0));
      canvas.focus();
      review.open();
  ...
  await run(
    "disabling or removing the focused review opener does not strand focus on body",
    async ({ bridge, review, canvas }) => {
      bridge.update(packet(0));
