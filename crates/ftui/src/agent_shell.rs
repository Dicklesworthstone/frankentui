...
//! Shared inline process-session model used by the `agent_shell` and `streaming` examples.
//!
//! This native subprocess reference model is not available on `wasm32`.
//!
//! [`StreamingOptions::parse_agent_shell`](crate::agent_shell::StreamingOptions::parse_agent_shell)
//! requires a child command and defaults to three rows with stdin enabled.
//! [`StreamingOptions::parse`](crate::agent_shell::StreamingOptions::parse) retains the
//! generated streaming demo with fifteen rows and opt-in child stdin.
//! Both parsers take arguments and environment values explicitly; this module
//! never reads process-global configuration or opens a terminal itself.
//!
//! The model keeps child lifecycle, pending previews, input, and log history
//! together. The executable owns [`crate::App`] assembly and chooses inline mode.
...
//! The streaming parser generates messages without arguments. A command after
//! `--` streams a real child's stdout/stderr through the same model and writer.
...
//! Streaming-demo child stdin is closed unless `--stdin` enables the editor;
//! the canonical agent shell enables it by default.
...
//! Three-row chrome reserves status/input/hint rows, without an input border
//! that would consume the editor's only row. The input echo reports admission
//! as `[stdin queued]`; it does not claim that the child has read those bytes.
...
//! Demo: `cargo run -p ftui-harness --example streaming`

use std::time::{Duration, Instant};

use crate::core::event::{Event, KeyCode, KeyEventKind, Modifiers, PasteEvent};
use crate::core::geometry::Rect;
use crate::layout::{Constraint, Flex};
use crate::render::frame::Frame;
use crate::render::sanitize::{SanitizeMode, sanitize};
use crate::runtime::{
    Cmd, Every, Model, ProcessControl, ProcessControlError, ProcessControlStatus, ProcessEvent,
    ProcessInput, ProcessInputError, ProcessInterruptStatus, ProcessSubscription, Subscription,
};
use crate::text::{Span, Text};
use crate::widgets::block::Block;
use crate::widgets::borders::{BorderType, Borders};
use crate::widgets::input::TextInput;
use crate::widgets::log_viewer::{LogViewer, LogViewerState};
use crate::widgets::paragraph::Paragraph;
use crate::widgets::status_line::{StatusItem, StatusLine};
use crate::widgets::{StatefulWidget, Widget};

const CHILD_LINKS_PER_RECORD: usize = 32;
/// Retained linked-record window and the executable's per-frame hyperlink limit.
pub const CHILD_LINK_RETENTION_LINES: usize = 200;
...
/// Reusable inline model for a child process or the generated streaming demo.
///
/// Construction creates local state; [`Model::subscriptions`] owns actual child
/// execution once the model is run by [`crate::App`].
...
/// Validated settings for the shared model and its inline terminal assembly.
///
/// [`Default`] retains the streaming demo's fifteen-row, noninteractive policy.
/// Use [`Self::parse_agent_shell`] for the canonical child-session defaults.
...
impl StreamingOptions {
    /// Parse the streaming demo CLI: optional child, fifteen rows, opt-in stdin.
    ///
    /// Explicit CLI values take precedence over the supplied environment values.
    /// No environment variables are read by this function.
    ///
    /// # Errors
    /// Returns [`std::io::ErrorKind::InvalidInput`] for malformed options or a
    /// missing command required by an option.
    pub fn parse(
        ...
        Self::parse_with_defaults(
            Self::default(),
            arguments,
            environment_log_mode,
            environment_exit_after_ms,
            false,
        )
    }

    /// Parse the canonical agent-shell CLI, requiring a command after `--`.
    ///
    /// Defaults are three rows, enabled child stdin, sanitized logs, and staying
    /// open after child exit. CLI values override the supplied environment values.
    ///
    /// # Errors
    /// Returns [`std::io::ErrorKind::InvalidInput`] for malformed options or a
    /// missing child command. Errors do not echo untrusted option contents.
    pub fn parse_agent_shell(
        arguments: impl IntoIterator<Item = String>,
        environment_log_mode: Option<std::ffi::OsString>,
        environment_exit_after_ms: Option<std::ffi::OsString>,
    ) -> std::io::Result<Self> {
    ...
    /// Configured inline height for the executable's [`crate::ScreenMode`].
    #[must_use]
    pub const fn ui_height(&self) -> u16 {
        self.ui_height
    }
    ...
    fn parse_with_defaults(
            ...
            match argument.as_str() {
                ...
                _ => {
                    return Err(std::io::Error::new(
                        std::io::ErrorKind::InvalidInput,
                        if require_command {
                            "usage: agent_shell [--exit-when-child-exits] [--log-mode=sanitized|sgr-only|raw] [--ui-height=N] [--exit-after-ms=N] -- COMMAND ARGS..."
                        } else {
        ...
        if require_command && options.command.is_empty() {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "agent_shell requires a command after --",
            ));
        }
...
/// Messages consumed by [`StreamingHarness`]. Runtime events convert through
/// [`From<Event>`]; subscription messages carry their originating run generation.
#[derive(Debug)]
pub enum Msg {
    ...
    Paste(PasteEvent),
    /// Generate the next demo batch.
    StreamTick,
    /// Child output or lifecycle notification scoped to its original run.
    Process {
        /// The originating [`ProcessControl`] generation.
        generation: u64,
        /// The child's output or lifecycle event.
        event: ProcessEvent,
    },
    /// Poll control completion and session time.
    PollControl,
    /// A terminal event that does not change this model.
    Noop,
}
...
impl StreamingHarness {
    /// Create local model state from validated options, without spawning a child
        ...
        match msg {
            Msg::Key(k) if matches!(k.kind, KeyEventKind::Press | KeyEventKind::Repeat) => {
                ...
                match k.code {
                    ...
                    KeyCode::Escape if self.child_finished => return Cmd::Quit,
...
mod tests {
    ...
    #[test]
    fn canonical_shell_defaults_and_overrides_preserve_literal_child_arguments() {
        let defaults = StreamingOptions::parse_agent_shell(
            ["--", "cat", "--log-mode=raw", "a b", "$(literal)"].map(str::to_owned),
            None,
            None,
        )
        .unwrap();
        assert_eq!(defaults.ui_height(), 3);
        assert!(defaults.stdin);
        assert_eq!(defaults.log_mode, SanitizeMode::Strip);
        assert!(!defaults.exit_when_child_exits);
        assert_eq!(defaults.exit_after, None);
        assert_eq!(
            defaults.command,
            ["cat", "--log-mode=raw", "a b", "$(literal)"]
        );

        let selected = StreamingOptions::parse_agent_shell(
            [
                "--ui-height=7",
                "--log-mode=sgr-only",
                "--exit-after-ms=250",
        ...
        .unwrap();
        assert_eq!(selected.ui_height(), 7);
        assert!(selected.stdin);
        assert_eq!(selected.log_mode, SanitizeMode::SgrOnly);
        assert!(selected.exit_when_child_exits);
        assert_eq!(selected.exit_after, Some(Duration::from_millis(250)));
        assert_eq!(selected.command, ["cat"]);
    }

    #[test]
    fn canonical_shell_escape_quits_only_after_child_finishes() {
        let options =
            StreamingOptions::parse_agent_shell(["--", "cat"].map(str::to_owned), None, None)
                .unwrap();
        let mut model = StreamingHarness::new(options);
        assert!(matches!(model.update(key(KeyCode::Escape)), Cmd::None));
        assert!(matches!(model.update(key(KeyCode::Char('q'))), Cmd::None));
        assert_eq!(model.input.value(), "q");
        let terminal = model.update(process_message(&model, ProcessEvent::Exited(7)));
        assert!(matches!(
            terminal,
            Cmd::Log { text, mode: SanitizeMode::Strip }
                if text == "[process] EXIT 7 lines=0"
        ));
        assert!(model.child_finished);
        assert!(matches!(model.update(key(KeyCode::Escape)), Cmd::Quit));
    }
}
